...
BACKLOG = 50
READ_TIMEOUT = 10.0
DB_FILE = "ts2database.txt"
LOG_FILE = "ts2responses.txt"
...
def handle_client(conn: socket.socket, addr, db: Dict[str, Tuple[str, str]], log_fp) -> None:
    ...
    try:
        while True:
            ...
            if line is None:
                # peer closed OR hit a read timeout without a full line
                return
...
def ts2(rudns_port_str: str):
    ...
    try:
        ...
        srv.bind(("", port))
        srv.listen(BACKLOG)
        print(f"[TS2] Listening on TCP {port} with {len(db)} records")
